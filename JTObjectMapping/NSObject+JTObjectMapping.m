    ...
    [dict enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        if (mapsToValue != nil) {
            if ([(NSObject *)mapsToValue isKindOfClass:[NSString class]]) {
                ...
                // TO DO: make this more generic and conform to JASTOR protocol (this is a quick hack!)
                NSDate *date = nil;
                BOOL shouldUseIsoFormat = YES;
                if( shouldUseIsoFormat ) {
                    JTISO8601DateFormatter *df = [[JTISO8601DateFormatter alloc] init];
                    date = [df dateFromString:obj];
                    [df release];
                }
                else {
